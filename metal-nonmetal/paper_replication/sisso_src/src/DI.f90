...
real*8,allocatable:: xinput(:,:,:),yinput(:,:),beta(:,:),beta_init(:,:),coeff(:,:),xprime(:,:,:),         xdprime(:,:,:),xmean(:,:),norm_xprime(:,:),yprime(:,:),prod_xty(:,:),prod_xtx(:,:,:),lassormse(:),         ymean(:),intercept(:),weight(:,:)
...
do i=1,ntask
  ...
  open(fileunit,file='SIS_subspaces/'//trim(adjustl(line)),status='old')
   ...
   call mpi_bcast(yinput,int(maxns*ntask,4),mpi_double_precision,0,mpi_comm_world,mpierr)
   call mpi_bcast(xinput,int(maxns*nf_DI*ntask,4),mpi_double_precision,0,mpi_comm_world,mpierr)
end do
...
call mpi_bcast(expr,int(nf_DI*200,4),mpi_character,0,mpi_comm_world,mpierr)
...
if(trim(adjustl(method_so))=='L1L0' .and. nf_DI > nf_L0 .and. ptype==1) then
    ...
    do i=1,ntask
       !xprime and xdprime
         ...
         prod_xtx(:,mpii,i)=matmul(xdprime(:nsample(i),mpii+sum(ncol(1:mpirank)),i)*weight(:nsample(i),i),                                             xdprime(:nsample(i),:,i))
         end do
    end do
    ...
    do ntry=1,L1para.nlambda
        ...
        ! lasso rmse
        do i=1,ntask
           lassormse(i)=sqrt(sum(weight(:nsample(i),i)*(yprime(:nsample(i),i)-matmul(xdprime(:nsample(i),:,i),                        beta(:,i)))**2)/nsample(i))  ! RMSE
        end do
...
integer nactive,activeset(:),i,j,k,l,loc(1),isc,ii(iFCDI),select_model(max(nmodel,1),iFCDI),         mID(max(nmodel,1),iFCDI),sc_loc(1),mpii,mpij
integer*8 totalm,bigm,nall,nrecord,nupper,nlower,njob(mpisize)
real*8 x(:,:,:),y(:,:),tmp,tmp2,wrmse(ntask),rmse(ntask),sc_rmse(2**iFCDI,ntask),maxae(ntask), sc_maxae(2**iFCDI,ntask),beta(iFCDI,ntask),intercept(ntask),select_rmse(max(nmodel,1)), select_score(max(nmodel,1),2,1+ntask),select_maxae(max(nmodel,1)), select_coeff(max(nmodel,1),iFCDI+1,ntask),mscore(max(nmodel,1),2,1+ntask), select_metric(max(nmodel,1)),mcoeff(max(nmodel,1),iFCDI+1,ntask),sc_beta(iFCDI,2**iFCDI,ntask), sc_intercept(2**iFCDI,ntask),sc_tmp(2**iFCDI),sc_tmp2(2**iFCDI),mpicollect(mpisize)
         ...
         do i=1,ntask
            if(.not. scmt) then
                 if(fit_intercept) then  ! linear fitting
                    call orth_de(x(:nsample(i),[activeset(ii(:iFCDI))],i),y(:nsample(i),i),                                  intercept(i),beta(:iFCDI,i),rmse(i))
                 else
                    call orth_de_nointercept(x(:nsample(i),[activeset(ii(:iFCDI))],i),y(:nsample(i),i),                                              beta(:iFCDI,i),rmse(i))
                    intercept(i)=0.d0
                 end if
                 ...
                 maxae(i)= maxval(abs(y(:nsample(i),i)-(intercept(i)+matmul(x(:nsample(i),                                   [activeset(ii(:iFCDI))],i),beta(:iFCDI,i)))))
            else  ! sign-constrained multi-task learning
                 if(fit_intercept) then
                    call sc_coord_descent(x(:nsample(i),[activeset(ii(:iFCDI))],i),y(:nsample(i),i),10000,1d-5,                          iFCDI,sc_intercept(:2**iFCDI,i),sc_beta(:iFCDI,:2**iFCDI,i),sc_rmse(:2**iFCDI,i))
                 else
                    call sc_coord_descent_nointercept(x(:nsample(i),[activeset(ii(:iFCDI))],i),y(:nsample(i),i),                          10000,1d-5,iFCDI,sc_beta(:iFCDI,:2**iFCDI,i),sc_rmse(:2**iFCDI,i))
                 ...
                 do isc=1,2**iFCDI
                    sc_maxae(isc,i)=maxval(abs(y(:nsample(i),i)-(sc_intercept(isc,i)+matmul(x(:nsample(i),                                   [activeset(ii(:iFCDI))],i),sc_beta(:iFCDI,isc,i)))))
                 end do
            end if
         end do
...
! collecting the best models
   if(mpirank>0) then
     ...
     do i=1,mpisize-1
       call mpi_recv(bigm,1,mpi_integer8,int(i,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
       totalm=totalm+bigm
     end do
   end if
   ...
   do j=1,min(int8(nmodel),totalm)  ! find the best models across all cores
      ...
      if(mpirank>0) then
        ...
        do i=1,mpisize-1
          call mpi_recv(mpicollect(i+1),1,mpi_double_precision,int(i,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        end do
      ...
      call mpi_bcast(mID(j,:iFCDI),int(iFCDI,4),mpi_integer,int(loc(1)-1,4),mpi_comm_world,mpierr)
      call mpi_bcast(mscore(j,:2,:),int(2*(1+ntask),4),mpi_double_precision,int(loc(1)-1,4),mpi_comm_world,mpierr)
      call mpi_bcast(mcoeff(j,:iFCDI+1,:),int((iFCDI+1)*ntask,4),mpi_double_precision,int(loc(1)-1,4),mpi_comm_world,mpierr)
   end do
   ...
   if(mpirank==0 .and. iFCDI==desc_dim) then !utput only the final dimension
       ...
       call writeout(iFCDI,mID(1,:iFCDI),mscore(1,1,2:1+ntask),mscore(1,2,2:1+ntask),                     mcoeff(1,2:,:),mcoeff(1,1,:),x,y,expr)
...
integer nactive,activeset(:),i,j,k,l,loc(1),ii(iFCDI),itask,mm1,mm2,mm3,mm4,ns,select_model(max(nmodel,1),iFCDI), mID(max(nmodel,1),iFCDI),overlap_n,overlap_n_tmp,select_overlap_n(max(nmodel,1)),nh,ntri,nconvexpair
integer*8 totalm,bigm,nall,nrecord,nlower,nupper
real*8  x(:,:,:),mscore(max(nmodel,1),2),overlap_size,overlap_size_tmp,select_overlap_size(max(nmodel,1)), hull(ubound(x,1),2),area(maxval(ngroup(:,1000))),mindist,xtmp1(ubound(x,1),3),xtmp2(ubound(x,1),3)
character expr(:)*200,line_name*100
...
! collecting the best models from all CPU cores
   ...
   do j=1,min(int8(nmodel),totalm)
      ...
      do k=1,min(int8(nmodel),totalm)
        if(select_overlap_n(k)==select_overlap_n(loc(1)) .and.            select_overlap_size(k)<select_overlap_size(loc(1))) loc(1)=k
      end do
      k=loc(1)
      ...
      if(mpirank>0) then
        ...
        do i=1,mpisize-1
          call mpi_recv(mpicollect(i+1,:),2,mpi_double_precision,int(i,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        end do
        loc=minloc(mpicollect(:,1))
        do l=1,mpisize
          if(mpicollect(l,1)==mpicollect(loc(1),1) .and.              mpicollect(l,2)<mpicollect(loc(1),2) ) loc(1)=l
        end do
        l=loc(1)
      end if
      ...
      call mpi_bcast(mID(j,:iFCDI),int(iFCDI,4),mpi_integer,int(l-1,4),mpi_comm_world,mpierr)
      call mpi_bcast(mscore(j,:),2,mpi_double_precision,int(l-1,4),mpi_comm_world,mpierr)
   end do
