...
if(nreject>0) call mpi_bcast(reject,int(nreject*str_len,4),mpi_character,0,mpi_comm_world,mpierr)
!---------------------------------
...
total_comb=0
call combine_se(inp,1_8,i,0_8,0_8,'NO',j,total_comb)
...
do icomb=1,rung
    ...
    ! mpin: store the Xi, mpin2: starting position of the Xi part in nthis
     ...
     ! broadcast ntot-nthis
       if(ntot>nthis) then
        i=ntot-nthis
        do ll=1,i
          call mpi_bcast(inp.Sexpr(ll).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,0,mpi_comm_world,mpierr)
          call mpi_bcast(inp.Sexpr(ll).list_len,1,mpi_integer,0,mpi_comm_world,mpierr)
          call mpi_bcast(inp.Sexpr(ll).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,0,mpi_comm_world,mpierr)
          call mpi_bcast(inp.Sexpr(ll).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,0,mpi_comm_world,mpierr)
          call mpi_bcast(inp.Sexpr(ll).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,0,mpi_comm_world,mpierr)
        end do
        call mpi_bcast(inp.feat_name(:i),int(i*str_len,4),mpi_character,0,mpi_comm_world,mpierr)
        call mpi_bcast(inp.lastop(:i),int(i*10,4),mpi_character,0,mpi_comm_world,mpierr)
        call mpi_bcast(inp.feat_unit(:,:i),int(i*nunit,4),mpi_double_precision,0,mpi_comm_world,mpierr)
        call mpi_bcast(inp.feat_comp(:i),int(i,4),mpi_integer8,0,mpi_comm_world,mpierr)
       end if
      ...
      if(mpirank==0) then  ! send nthis
        do i=2,mpisize
          ...
          if(mpin(i)>0) then
          do ll=j,ntot
           call mpi_send(inp.Sexpr(ll).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,int(i-1,4),5,mpi_comm_world,mpierr)
           call mpi_send(inp.Sexpr(ll).list_len,1,mpi_integer,int(i-1,4),6,mpi_comm_world,mpierr)
           call mpi_send(inp.Sexpr(ll).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,int(i-1,4),7,mpi_comm_world,mpierr)
           call mpi_send(inp.Sexpr(ll).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,int(i-1,4),8,mpi_comm_world,mpierr)
           call mpi_send(inp.Sexpr(ll).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,int(i-1,4),9,mpi_comm_world,mpierr)
           call mpi_recv(mpisync,1,mpi_character,int(i-1,4),10,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
          end do
          call mpi_send(inp.feat_name(j:ntot),int(k*str_len,4),mpi_character,int(i-1,4),1,mpi_comm_world,mpierr)
          call mpi_send(inp.lastop(j:ntot),int(k*10,4),mpi_character,int(i-1,4),2,mpi_comm_world,mpierr)
          call mpi_send(inp.feat_comp(j:ntot),int(k,4),mpi_integer8,int(i-1,4),3,mpi_comm_world,mpierr)
          call mpi_send(inp.feat_unit(:,j:ntot),int(k*nunit,4),mpi_double_precision,int(i-1,4),4,mpi_comm_world,mpierr)
          end if
        end do
      else  ! receive
        ...
        if(mpin(mpirank+1)>0) then
          do ll=j,l
           call mpi_recv(inp.Sexpr(ll).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,0,5,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
           call mpi_recv(inp.Sexpr(ll).list_len,1,mpi_integer,0,6,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
           call mpi_recv(inp.Sexpr(ll).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,0,7,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
           call mpi_recv(inp.Sexpr(ll).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,0,8,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
           call mpi_recv(inp.Sexpr(ll).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,0,9,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
           call mpi_send(mpisync,1,mpi_character,0,10,mpi_comm_world,mpierr)
          end do
          call mpi_recv(inp.feat_name(j:l),int(k*str_len,4),mpi_character,0,1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
          call mpi_recv(inp.lastop(j:l),int(k*10,4),mpi_character,0,2,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
          call mpi_recv(inp.feat_comp(j:l),int(k,4),mpi_integer8,0,3,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
          call mpi_recv(inp.feat_unit(:,j:l),int(k*nunit,4),mpi_double_precision,0,4,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        end if
      end if
      ...
      total_comb=mpin(mpirank+1)*(ntot-nthis)+(mpin(mpirank+1)-1)*mpin(mpirank+1)/2.0+                  mpin(mpirank+1)*(nthis-mpin2(mpirank+1)+1-mpin(mpirank+1))+mpin(mpirank+1) ! binary+unary

      if(mpin(mpirank+1)>0)       call combine_se(inp,i,j,1_8,k,trim(adjustl(ops(icomb))),nf(icomb),total_comb)
      call mpi_barrier(mpi_comm_world,mpierr)
  ...
  IF (icomb < rung) THEN
       ...
       if(mpirank/=0) then
          ...
          do mpii=1,mpisize-1
               call mpi_recv(nfpcore_this(mpii+1),1,mpi_integer8,int(mpii,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
          end do
       end if
       call mpi_bcast(nfpcore_this,int(mpisize,4),mpi_integer8,0,mpi_comm_world,mpierr)
      ...
      !--------------------------------------------------------------------
       ...
       if(mpirank>0 .and. i>0) then
          do ll=1,i
           call mpi_send(gen.Sexpr(ll).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,0,5,mpi_comm_world,mpierr)
           call mpi_send(gen.Sexpr(ll).list_len,1,mpi_integer,0,6,mpi_comm_world,mpierr)
           call mpi_send(gen.Sexpr(ll).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,0,7,mpi_comm_world,mpierr)
           call mpi_send(gen.Sexpr(ll).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,0,8,mpi_comm_world,mpierr)
           call mpi_send(gen.Sexpr(ll).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,0,9,mpi_comm_world,mpierr)
           call mpi_recv(mpisync,1,mpi_character,0,10,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
          end do
          call mpi_send(gen.feat_name(:i),int(i*str_len,4),mpi_character,0,1,mpi_comm_world,mpierr)
          call mpi_send(gen.lastop(:i),int(i*10,4),mpi_character,0,2,mpi_comm_world,mpierr)
          call mpi_send(gen.feat_comp(:i),int(i,4),mpi_integer8,0,3,mpi_comm_world,mpierr)
          call mpi_send(gen.feat_unit(:,:i),int(nunit*i,4),mpi_double_precision,0,4,mpi_comm_world,mpierr)
          ...
          do mpii=1,mpisize-1
              ...
              if(l>0) then
              do ll=j,k
         call mpi_recv(inp.Sexpr(ll).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,int(mpii,4),5,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
         call mpi_recv(inp.Sexpr(ll).list_len,1,mpi_integer,int(mpii,4),6,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
         call mpi_recv(inp.Sexpr(ll).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,int(mpii,4),7,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
         call mpi_recv(inp.Sexpr(ll).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,int(mpii,4),8,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
         call mpi_recv(inp.Sexpr(ll).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,int(mpii,4),9,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
         call mpi_send(mpisync,1,mpi_character,int(mpii,4),10,mpi_comm_world,mpierr)
              end do
              call mpi_recv(inp.feat_name(j:k),int(l*str_len,4),mpi_character,int(mpii,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.lastop(j:k),int(l*10,4),mpi_character,int(mpii,4),2,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.feat_comp(j:k),int(l,4),mpi_integer8,int(mpii,4),3,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.feat_unit(:,j:k),int(nunit*l,4),mpi_double_precision,int(mpii,4),4,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              end if
         end do
        end if
       ...
       !----------------------------------------------------------------
       if(mpirank/=0) then
         call mpi_send(sel.nselect,1,mpi_integer8,0,5,mpi_comm_world,mpierr)
       else
         mpik=sel.nselect
         do mpii=1,mpisize-1
           call mpi_recv(mpij,1,mpi_integer8,int(mpii,4),5,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
...
if(rung==0) then
 ...
 if(mpirank/=0) then
     ...
     call mpi_send(sel.nselect,1,mpi_integer8,0,2,mpi_comm_world,mpierr)
 else
      nfpcore_this(1)=sel.nselect
      do mpii=1,mpisize-1
           call mpi_recv(mpij,1,mpi_integer8,int(mpii,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
           nf(rung)=nf(rung)+mpij
           call mpi_recv(nfpcore_this(mpii+1),1,mpi_integer8,int(mpii,4),2,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
      end do
   ...
   if(ptype==1) then
      ...
      do i=1,nf_sis_avai(iFCDI)
        ...
        if(abs(sis.feat_score(2,i))>1d9) then
           ...
           write(fileunit,'(2a,i6,a,f12.4)') trim(sis.feat_name(i)),'    N_overlap =',nint(sis.feat_score(1,i)),                                                                    '    S_overlap =',sis.feat_score(2,i)
        end if
      end do
   end if
   close(fileunit)
...
600   continue
      ...
      ! multiplication and division
          ...
          ! division
           IF(index(myops,'(/)')/=0) then
              ...
              if(index(myops,'(*)')/=0 .and.                 (index(myinp.lastop(i),'(/)')/=0 .or. index(myinp.lastop(j),'(/)')/=0) .and.                 (icomb < rung)  )  goto 602   ! do nothing
              ...
              if(index(myinp.lastop(i),'(*)')/=0) then
                 ...
                 if(k>0) then
                  ...
                  if( (trim(adjustl(myinp.feat_name(i)(:k-1)))=='(' &   ! j being the left part of i
                      .and. myinp.feat_name(i)(k+kkk:k+kkk)=='*') .or.                       (kk==kkk+1 .and. myinp.feat_name(i)(k-1:k-1)=='*')) then ! j being the right part of i
             ...
             if(index(myinp.lastop(j),'(*)')/=0) then
                ...
                if(k>0) then
                  ...
                  if((trim(adjustl(myinp.feat_name(j)(:k-1)))=='('                       .and. myinp.feat_name(j)(k+kkk:k+kkk)=='*') .or. &   ! i being the left part of j
  ...
  if(float(counter)/float(total_comb)>=progress) then
     ...
     write(*,'(a,i4,2(a,i15),a,f6.1,a)')      'mpirank = ',mpirank,'  #generated =',nf,'  #selected =',sel.nselect,'  progress =',progress*100,'%'
     progress=progress+0.2
  end if
...
if(complexity1<complexity2) then
   ...
   if(len_trim(name1)<len_trim(name2) .or.      (len_trim(name1)==len_trim(name2) .and. trim(name1)<=trim(name2) ) ) then
      simpler_se=1
   else
      simpler_se=2
   end if
...
real*8 fID(:)
integer*8 i,j,k,l,ll,mpii,mpij,nfpcore_this(:),mpim(mpisize),mpiloc(mpisize),order(:),loc(1),           complexity(:),simpler_result
character(len=*) fname(:)
logical available(:)
...
IF(mpisize>1) THEN
   ...
   do k=1,mpisize-1
     ...
     if(nfpcore_this(mpiloc(k))>0) then
         ...
         ! broadcast compidentity from core mpiloc(k)-1 to all other cores
          call mpi_bcast(compidentity,int(nfpcore_this(mpiloc(k)),4),mpi_double_precision,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
         ! broadcast the feature expressions
          call mpi_bcast(compname,int(nfpcore_this(mpiloc(k))*str_len,4),mpi_character,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
         ! broadcast the feature complexities
          call mpi_bcast(compcomplexity,int(nfpcore_this(mpiloc(k)),4),mpi_integer8,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
         ! broadcast the features
          do ll=1,nfpcore_this(mpiloc(k))
           call mpi_bcast(Sexpr4comp(ll).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
           call mpi_bcast(Sexpr4comp(ll).list_len,1,mpi_integer,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
           call mpi_bcast(Sexpr4comp(ll).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
           call mpi_bcast(Sexpr4comp(ll).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
           call mpi_bcast(Sexpr4comp(ll).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
          end do
         ...
         if( all( mpirank/=(mpiloc(:k)-1) ) .and. nfpcore_this(mpirank+1)>0 ) then ! this core vs core mpiloc(k)-1
            do mpij=1,nfpcore_this(mpiloc(k))  ! core mpiloc(k)-1
              ...
              simpler_result=simpler_se(compcomplexity(mpij),complexity(order(i)),compname(mpij),fname(order(i)))
                 ...
                 ! if not equal
                 if(compidentity(mpij)>fID(order(i)) .or.  (compidentity(mpij)==fID(order(i)) .and.                     simpler_result==1 ) ) then   ! mpij is better than i
            ...
            call mpi_send(compidentity,int(nfpcore_this(mpiloc(k)),4),mpi_double_precision,int(mpiloc(k)-1,4),33,mpi_comm_world,mpierr)

         else if(mpirank==mpiloc(k)-1) then
             do l=1,mpisize
                 if( any( l==mpiloc(:k) ) .or. nfpcore_this(l)==0 ) cycle
            call mpi_recv(compidentity,int(nfpcore_this(mpiloc(k)),4),mpi_double_precision,mpi_any_source,33,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
  ...
  if(mpirank/=0)  then
    ...
    do k=1,mpisize-1
      call mpi_recv(i,1,mpi_integer8,int(k,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
      j=j+i
    end do
  end if
...
do l=1,mpisize
 call mpi_bcast(pavailable(l),1,mpi_logical,int(l-1,4),mpi_comm_world,mpierr)
end do

do k=1,nfpcore_this(mpirank+1)
  if(.not. available(k)) sel.feat_score(:,k)=-1.0
end do
...
do while( any(pavailable) .and. i<nf_sis(iFCDI) )
      ...
      do l=1,nfpcore_this(mpirank+1)  ! equal scores
        if(  abs(tmp-sel.feat_score(1,l))<=1d-8 ) then
           if( sel.feat_score(2,l)-sel.feat_score(2,loc(2))>1d-8 .or.                (abs(sel.feat_score(2,l)-sel.feat_score(2,loc(2)))<=1d-8 .and.             simpler_se(sel.feat_comp(l),sel.feat_comp(loc(2)),sel.feat_name(l),sel.feat_name(loc(2)))==1 ) )  loc(2)=l
        end if
      end do
      if(nfpcore_this(mpirank+1)>0) then
      ...
      do l=1,mpisize  ! inform other cores about local best features
        call mpi_bcast(pscore(:,l),2,mpi_double_precision,int(l-1,4),mpi_comm_world,mpierr)
        call mpi_bcast(pcomplexity(l),1,mpi_integer8,int(l-1,4),mpi_comm_world,mpierr)
        call mpi_bcast(pname(l),int(str_len,4),mpi_character,int(l-1,4),mpi_comm_world,mpierr)
      end do
      ...
      do l=1,mpisize  ! if equal score between features
        if(  abs(tmp-pscore(1,l))<=1d-8 ) then
           if( pscore(2,l)-pscore(2,loc(1))>1d-8 .or.  ( abs(pscore(2,l)-pscore(2,loc(1)))<=1d-8 .and.                simpler_se(pcomplexity(l),pcomplexity(loc(1)),pname(l),pname(loc(1)))==1 ) ) loc(1)=l
        end if
      end do
      ...
      if((loc(1)-1)==mpirank) then
        ...
        call mpi_send(sel.Sexpr(loc(2)).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,0,96,mpi_comm_world,mpierr)
        call mpi_send(sel.Sexpr(loc(2)).list_len,1,mpi_integer,0,97,mpi_comm_world,mpierr)
        call mpi_send(sel.Sexpr(loc(2)).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,0,98,mpi_comm_world,mpierr)
        call mpi_send(sel.Sexpr(loc(2)).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,0,99,mpi_comm_world,mpierr)
        call mpi_send(sel.Sexpr(loc(2)).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,0,100,mpi_comm_world,mpierr)
        call mpi_send(sel.feat_name(loc(2)),int(str_len,4),mpi_character,0,101,mpi_comm_world,mpierr)
        call mpi_send(sel.feat_score(:,loc(2)),2,mpi_double_precision,0,103,mpi_comm_world,mpierr)
         end if
     ...
     call mpi_recv(sis.Sexpr(i).list_id(:Smaxlen),int(Smaxlen,4),mpi_integer,int(loc(1)-1,4),96,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
     call mpi_recv(sis.Sexpr(i).list_len,1,mpi_integer,int(loc(1)-1,4),97,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
     call mpi_recv(sis.Sexpr(i).list_pointer(:2,:Smaxlen),int(Smaxlen*2,4),mpi_integer,int(loc(1)-1,4),98,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
     call mpi_recv(sis.Sexpr(i).list_var(:Smaxlen),int(Smaxlen*30,4),mpi_character,int(loc(1)-1,4),99,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
     call mpi_recv(sis.Sexpr(i).list_op(:Smaxlen),int(Smaxlen*10,4),mpi_character,int(loc(1)-1,4),100,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
     call mpi_recv(sis.feat_name(i),int(str_len,4),mpi_character,int(loc(1)-1,4),101,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
     call mpi_recv(sis.feat_score(:,i),2,mpi_double_precision,int(loc(1)-1,4),103,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
      end if
      !---
...
if(mpirank/=0)  then
  ...
  do l=1,mpisize-1
    call mpi_recv(i,1,mpi_integer8,int(l,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
    n=n+i
 end do
end if
...
real*8 feat(npoint),sdfeat(npoint),tmp(ntask),sis_score_se(2),yyy(:),xnorm(ntask),xmean(ntask),        overlap_length,length_tmp,feat_tmp1(npoint),feat_tmp2(npoint),mindist,minlen
logical isoverlap
type(Sexpression) mySexpr
...
do i=2,sel.nselect   ! compare feaure i with the j located at middle of order(1:n)
  ...
  ! bisection method for descending order
  if( (sel.feat_score(1,i)>sel.feat_score(1,order(j))) .or.  ((sel.feat_score(1,i)==sel.feat_score(1,order(j)))        .and. sel.feat_score(2,i)>sel.feat_score(2,order(j))) .or.  ((sel.feat_score(1,i)==sel.feat_score(1,order(j)))        .and. sel.feat_score(2,i)==sel.feat_score(2,order(j)) .and. simpler_result==1) )then
...
k=mySexpr.list_len
do i=1,k
write(*,'(a,i3.3,a,a,2i5,2a)') '(',mySexpr.list_id(i),')  ',       trim(mySexpr.list_op(i)),mySexpr.list_pointer(:,i),'  ',trim(mySexpr.list_var(i))
end do
end subroutine
