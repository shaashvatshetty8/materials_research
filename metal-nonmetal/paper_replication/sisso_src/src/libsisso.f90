...
! inverse: inverse of a given matrix
...
! string_split: breaking a string into sub-strings.
! kfoldCV: k-fold CV for linear model
! convex2d_xxx: construction of 2D convex hull
! intriangle: checking if a 3D point is inside a triangle
! convex3d_xxx: construction of 3D convex hull
! interlp: intersection between a line and a plane
...
real*8 interlp(3),p1(3),p2(3),p3(3),p4(3),p5(3),normal(3),a,b,c,d,        m,n,p,t
! for plane
...
n=ubound(mat,1)
lm=0;x=0;y=0
if (abs(det(mat))==0) then
  print *, 'Error: Value of the determinant is 0, the matrix is singular'; stop
end if
if (abs(det(mat))<1d-30) write(*,'(a,E15.6E3,a)') 'Warning: Value of the determinant is:', det(mat),' matrix might be singular'
...
implicit none
real*8  x(:,:),y(:),beta(ubound(x,2)),xprime(ubound(x,1),ubound(x,2)),yprime(ubound(y,1)),intercept,wrmse,rmse, q(ubound(x,1),ubound(x,2)),r(ubound(x,2),ubound(x,2)),qty(ubound(x,2)),xmean(ubound(x,2)),ymean,weight(:), wx(ubound(x,1),ubound(x,2)),wy(ubound(y,1))
integer i,j,k,m,n
...
! https://en.wikipedia.org/wiki/Linear_least_squares_(mathematics)
implicit none
real*8  x(:,:),y(ubound(x,1)),beta0(ubound(x,2)+1),rmse,x0(ubound(x,1),ubound(x,2)+1),         beta(ubound(x,2)),intercept
integer m,n
m=ubound(x,1)
n=ubound(x,2)
...
implicit none
real*8 prod_xty(:),prod_xtx(:,:),lambda,beta(:),beta_init(:), beta_old(ubound(beta,1)),z,tole,rand(ubound(beta,1)),beta_tmp,dbeta,xxbeta(ubound(beta,1))
...
real*8 prod_xty(:,:),prod_xtx(:,:,:),lambda,beta(:,:),beta_init(:,:),beta_old(ubound(beta,1),ubound(beta,2)),tole, beta_tmp(ubound(beta,2)),beta_tmp2(ubound(beta,2)),dbeta,xxbeta(ubound(beta,1),ubound(beta,2)), Sj(ubound(beta,2)),norm
...
mpin=ncol(mpirank+1)
if (mpirank /=0) then
call mpi_send(xxbeta(1:mpin,:),int(mpin*ntask,4),mpi_double_precision,0,1,mpi_comm_world,mpierr)
else
do mpii=1,mpisize-1
mpij=sum(ncol(1:mpii))
mpik=ncol(mpii+1)
call mpi_recv(xxbeta(mpij+1:mpij+mpik,:),int(mpik*ntask,4),mpi_double_precision,int(mpii,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
end do
end if

call mpi_barrier(mpi_comm_world,mpierr)
call mpi_bcast(xxbeta,int(ntotf*ntask,4),mpi_double_precision,0,mpi_comm_world,mpierr)
...
do i=1,max_iter
  ...
  if(mpirank/=0) then  ! do the job processor after processor,core0->core1->core3...
   call mpi_recv(xxbeta,int(ntotf*ntask,4),mpi_double_precision,int(mpirank-1,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
  end if
 ...
 call mpi_send(xxbeta,int(ntotf*ntask,4),mpi_double_precision,int(mpirank+1,4),1,mpi_comm_world,mpierr)
 end if
! core0->core1, core1->core2, core2->core3, ..., core(n-1)->core(n)

 call mpi_barrier(mpi_comm_world,mpierr)
 call mpi_bcast(xxbeta,int(ntotf*ntask,4),mpi_double_precision,int(mpisize-1,4),mpi_comm_world,mpierr)

  mpin=ncol(mpirank+1)
  if (mpirank /=0) then
  call mpi_send(beta(1+sum(ncol(1:mpirank)):sum(ncol(1:mpirank+1)),:),int(ncol(mpirank+1)*ntask,4),mpi_double_precision,0,1,mpi_comm_world,mpierr)
  else
  do mpii=1,mpisize-1
  mpij=sum(ncol(1:mpii))
  mpik=ncol(mpii+1)
  call mpi_recv(beta(mpij+1:mpij+mpik,:),int(mpik*ntask,4),mpi_double_precision,int(mpii,4),1,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
  end do
  end if
  call mpi_bcast(beta,int(ntotf*ntask,4),mpi_double_precision,0,mpi_comm_world,mpierr)
  call mpi_barrier(mpi_comm_world,mpierr)
...
real*8 x(:,:),y(ubound(x,1)),beta(ubound(x,2),2**dd),beta_old(ubound(x,2),2**dd),xprime(ubound(x,1),ubound(x,2)), xdprime(ubound(x,1),ubound(x,2)),yprime(ubound(y,1)),intercept(2**dd),rmse(2**dd),tole,norm_beta, prod_xty(ubound(x,2)),prod_xtx(ubound(x,2),ubound(x,2)),xpnorm(ubound(x,2)),ymean,xmean(ubound(x,2))
...
implicit none
integer m,n,i,j,k,max_iter,dd
real*8 x(:,:),y(ubound(x,1)),beta(ubound(x,2),2**dd),beta_old(ubound(x,2),2**dd),xprime(ubound(x,1),ubound(x,2)), rmse(2**dd),tole,norm_beta,prod_xty(ubound(x,2)),prod_xtx(ubound(x,2),ubound(x,2)),xnorm(ubound(x,2))
integer csign(ubound(x,2),2**dd)
...
implicit none
real*8 x(:,:),y(ubound(x,1)),beta(ubound(x,2)),beta_old(ubound(x,2)),xprime(ubound(x,1),ubound(x,2)), xdprime(ubound(x,1),ubound(x,2)),yprime(ubound(y,1)),intercept,rmse,tole,norm_beta,prod_xty(ubound(x,2)), prod_xtx(ubound(x,2),ubound(x,2)),xpnorm(ubound(x,2)),ymean,xmean(ubound(x,2))
...
do l=1,fold
   ...
   if(mm1==mm3) then
      ...
      call orth_de(x([random(mm1:mm3-1),random(mm4+1:mm2)],:),                    y([random(mm1:mm3-1),random(mm4+1:mm2)])+noise([random(mm1:mm3-1),random(mm4+1:mm2)]),                    intercept,beta(:),rmse)
   end if
...
do i=1,ntot
   ...
   if( sqrt(sum((set(edges(1,1),:)-set(i,:))**2))<1d-10 .or. sqrt(sum((set(edges(1,2),:)-set(i,:))**2))<1d-10)       overlap(i)=.true.
end do
  ...
  if(goodedge) then
      ! add new triangle
       ...
       do i=1,ntot
          ...
          if( (abs(set(k,1)-set(i,1))<=1d-10 .and. abs(set(k,2)-set(i,2))<=1d-10 .and.               abs(set(k,3)-set(i,3))<=1d-10 )) overlap(i)=.true.
       end do
...
do i=1,ntri
  ...
  do j=i+1,ntri  ! find the coplane triangles if there are
     ...
     if(sqrt(sum(crosspro_abnormalized(normal_a,normal_b)**2))<=1d-10 .and.         dispp(set(triangles(j,1),:),set(triangles(i,1),:),set(triangles(i,2),:),set(triangles(i,3),:))<1d-6) then
...
real*8 set1(:,:),set2(:,:),area,hull1(ubound(set1,1),2),hull2(ubound(set2,1),2),bwidth,norm1,norm2,vunit1(2),vunit2(2), set3(10*(ubound(set1,1)+ubound(set2,1)),2),tmp,segp(4,2),delta,lambda,mu,xa,xb,ya,yb,xc,xd,yc,yd
...
if((ns1>2 .and. (.not. polygon1)) .or. (ns2>2 .and. (.not. polygon2)) ) then   ! two lines
  ...
  do i=1,ns2
    ...
    do j=1,nh1
       ...
       if(tmp>0.d0) then
          if(bwidth>0.d0) then
            if(convex2d_dist(set2(i:i,:2),hull1(:nh1,:2))>bwidth) then  ! boundary tolerance
             inside=.false.
             exit
            end if
          else
  ...
  do i=1,ns1
    ...
    do j=1,nh2
       ...
       if(tmp>0.d0) then  ! point on the left side of the edge
          if(bwidth>0.d0) then
             if(convex2d_dist(set1(i:i,:2),hull2(:nh2,:2))>bwidth) then  ! boundary tolerance
                inside=.false.
                exit
             end if
          else
...
do i=1,np
    ...
    do j=1,nh
       ...
       if(tmp>0.d0) then
            if(bwidth>0.d0) then
               if(convex2d_dist(set2(i:i,:2),hull(:nh,:2))>bwidth) then  ! boundary tolerance
                  convex2d_in=convex2d_in-1
                  exit
               end if
            else
...
real*8 set1(:,:),set2(:,:),convex2d_dist,hull1(ubound(set1,1),2),hull2(ubound(set2,1),2), p1(2),p2(2),p3(2),va(2),vb(2),vv(2),dot,len_sq,param,dist
...
mini=minval(set2)-bwidth
maxi=maxval(set2)+bwidth
do i=1,ns1
 if(set1(i)>=mini .and. set1(i)<=maxi) numb=numb+1
end do

mini=minval(set1)-bwidth
...
real*8 pref(3),set1(:,:),set2(:,:),bwidth,pproj(3),pinter(3),v1(3),v2(3),v3(3),        normal(3),ddd,ttt,area,dist(3),vtmp(3,3)
...
do iii=1,ubound(set2,1)
     ...
     do i=1,ntri
       ...
       if(sqrt(sum((set2(iii,:)-pproj)**2))<=bwidth) then
          if(intriangle(pproj,set1(triangles(i,1),:),set1(triangles(i,2),:),set1(triangles(i,3),:))) then !projection inside
             ...
             if(sum(vtmp(1,:)*v1) <=0.d0 .or.                 sum(vtmp(2,:)*(-v1)) <=0.d0 ) then
