...
use FC
use DI
! use ifport  ! Intel-specific; replaced with execute_command_line below
!-------------------
implicit none
...
if(restart==1) then
  ...
  tcontinue='FC'
  if(mpirank==0) then
     call execute_command_line('rm -rf Models SIS_subspaces')
     call execute_command_line('mkdir -p Models')
     call execute_command_line('mkdir -p Models/data_top1')
     call execute_command_line('mkdir -p SIS_subspaces')
...
scmt=.false.          
desc_dim=2           
! nsample=5  ! removed: nsample is allocated later in main; gfortran can't scalar-assign to unallocated array
nsf= 5                
fcomplexity=3         
nunit=1
nf_sis=50000
