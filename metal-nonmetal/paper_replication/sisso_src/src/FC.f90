...
total_comb=0
call combine(inp,1_8,i,0_8,0_8,'NO',j,total_comb)
...
do icomb=1,rung
    ...
    ! mpin: store the Xi, mpin2: starting position of the Xi part in nthis
     ...
     ! check if array size need to be increased
      ...
      if(ubound(inp.feat_data,2)< i)         call addm_inp(i-ubound(inp.feat_data,2),inp)

     ! broadcast ntot-nthis
       if(ntot>nthis) then
        i=ntot-nthis
        call mpi_bcast(inp.feat_data(:,:i),int(i*npoint,4),mpi_double_precision,0,mpi_comm_world,mpierr)
      ...
      if(mpirank==0) then  ! send nthis
        do i=2,mpisize
          ...
          if(mpin(i)>0) then
          call mpi_send(inp.feat_data(:,j:ntot),int(k*npoint,4),mpi_double_precision,int(i-1,4),31,mpi_comm_world,mpierr)
          call mpi_send(inp.feat_name(j:ntot),int(k*str_len,4),mpi_character,int(i-1,4),32,mpi_comm_world,mpierr)
          call mpi_send(inp.lastop(j:ntot),int(k*10,4),mpi_character,int(i-1,4),33,mpi_comm_world,mpierr)
          call mpi_send(inp.feat_comp(j:ntot),int(k,4),mpi_integer8,int(i-1,4),34,mpi_comm_world,mpierr)
          call mpi_send(inp.feat_unit(:,j:ntot),int(k*nunit,4),mpi_double_precision,int(i-1,4),35,mpi_comm_world,mpierr)
          end if
        end do
      else  ! receive
        ...
        call mpi_recv(inp.feat_data(:,j:l),int(k*npoint,4),mpi_double_precision,0,31,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        call mpi_recv(inp.feat_name(j:l),int(k*str_len,4),mpi_character,0,32,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        call mpi_recv(inp.lastop(j:l),int(k*10,4),mpi_character,0,33,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        call mpi_recv(inp.feat_comp(j:l),int(k,4),mpi_integer8,0,34,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        call mpi_recv(inp.feat_unit(:,j:l),int(k*nunit,4),mpi_double_precision,0,35,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
        end if
      end if
      ...
      if(mpin(mpirank+1)>0)       call combine(inp,i,j,1_8,k,trim(adjustl(ops(icomb))),nf(icomb),total_comb)
  ...
  IF (icomb < rung) THEN
      ...
      !--------------------------------------------------------------------
       ...
       if(mpirank>0 .and. i>0) then
          call mpi_send(gen.feat_data(:,:i),int(npoint*i,4),mpi_double_precision,0,2,mpi_comm_world,mpierr)
          call mpi_send(gen.feat_name(:i),int(i*str_len,4),mpi_character,0,3,mpi_comm_world,mpierr)
          call mpi_send(gen.lastop(:i),int(i*10,4),mpi_character,0,4,mpi_comm_world,mpierr)
          call mpi_send(gen.feat_comp(:i),int(i,4),mpi_integer8,0,5,mpi_comm_world,mpierr)
          call mpi_send(gen.feat_unit(:,:i),int(nunit*i,4),mpi_double_precision,0,6,mpi_comm_world,mpierr)
          ...
          do mpii=1,mpisize-1
              ...
              if(l>0) then
              call mpi_recv(inp.feat_data(:,j:k),int(npoint*l,4),mpi_double_precision,int(mpii,4),2,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.feat_name(j:k),int(l*str_len,4),mpi_character,int(mpii,4),3,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.lastop(j:k),int(l*10,4),mpi_character,int(mpii,4),4,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.feat_comp(j:k),int(l,4),mpi_integer8,int(mpii,4),5,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              call mpi_recv(inp.feat_unit(:,j:k),int(nunit*l,4),mpi_double_precision,int(mpii,4),6,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
              end if
         end do
        end if
   ...
   do k=1,mpisize-1
     ...
     if(nfpcore_this(mpiloc(k))>0) then
         ...
         ! broadcast the features
          call mpi_bcast(compfeat,int(npoint*nfpcore_this(mpiloc(k)),4),mpi_double_precision,int(mpiloc(k)-1,4),mpi_comm_world,mpierr)
         ...
         if( all( mpirank/=(mpiloc(:k)-1) ) .and. nfpcore_this(mpirank+1)>0 ) then ! this core vs core mpiloc(k)-1
            do mpij=1,nfpcore_this(mpiloc(k))  ! core mpiloc(k)-1
              ...
              simpler_result=simpler(compcomplexity(mpij),complexity(order(i)),compname(mpij),fname(order(i)))
                 ...
                 if(equivalent(compidentity(mpij),fID(order(i)),compfeat(:npoint,mpij),                                feat(:npoint,order(i))) ) then
...
do while( any(pavailable) .and. i<nf_sis(iFCDI) )
      ...
      do l=1,nfpcore_this(mpirank+1)  ! equal scores
        if(  abs(tmp-sel.feat_score(1,l))<=1d-8 ) then
           if( sel.feat_score(2,l)-sel.feat_score(2,loc(2))>1d-8 .or.              (abs(sel.feat_score(2,l)-sel.feat_score(2,loc(2)))<=1d-8 .and.              simpler(sel.feat_comp(l),sel.feat_comp(loc(2)),sel.feat_name(l),sel.feat_name(loc(2)))==1) ) loc(2)=l
        end if
      end do
      if(nfpcore_this(mpirank+1)>0) then
      ...
      do l=1,mpisize  ! if equal score between features
        if(  abs(tmp-pscore(1,l))<=1d-8 ) then
           if( pscore(2,l)-pscore(2,loc(1))>1d-8 .or.  ( abs(pscore(2,l)-pscore(2,loc(1)))<=1d-8 .and.                simpler(pcomplexity(l),pcomplexity(loc(1)),pname(l),pname(loc(1)))==1 ) ) loc(1)=l
        end if
      end do
      ...
      if((loc(1)-1)==mpirank) then
         if(mpirank==0) then
            ...
            call mpi_send(sel.feat_data(:,loc(2)),int(npoint,4),mpi_double_precision,0,100,mpi_comm_world,mpierr)
      ...
      if(mpirank==0 .and. mpirank/=(loc(1)-1) ) then
           call mpi_recv(sis.feat_data(:,i),int(npoint,4),mpi_double_precision,int(loc(1)-1,4),100,mpi_comm_world,MPI_STATUS_IGNORE,mpierr)
...
real*8 feat(:),sdfeat(ubound(feat,1)),tmp(ntask),sis_score(2),yyy(:),xnorm(ntask),xmean(ntask),        overlap_length,length_tmp,feat_tmp1(ubound(feat,1)),feat_tmp2(ubound(feat,1)),mindist,minlen
logical isoverlap
...
do i=2,sel.nselect   ! compare feaure i with the j located at middle of order(1:n)
  ...
  if(equivalent(sel.feat_score(1,i),sel.feat_score(1,order(j)),sel.feat_data(:npoint,i),                 sel.feat_data(:npoint,order(j))) ) then
